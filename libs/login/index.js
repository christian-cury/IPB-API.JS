    ...
    */
	this.getLoginData = function(req, callback) {
		...
		if(req.body.username && req.body.password) {
			...
			API.Database.get("members", {where: "name = :name", values: {name: req.body.username} }, function(result){
			...
			console.log("Login via Loginkey");
			API.Database.get("members", {where: "member_login_key = :key", values: {key: req.body.loginkey} }, function(result){
				return callback(result);
			});
		}
